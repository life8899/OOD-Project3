...
*/
ClientHandler::ClientHandler(BlockingQueue<Message>* pQ, const std::string& name) : pQ_(pQ), clientHandlerName(name)
{
...
void ClientHandler::setQueue(BlockingQueue<Message>* pQ)
{
	pQ_ = pQ;
}
//----< frame message string by reading bytes from socket >--------

std::string ClientHandler::readMsg(Socket& socket)
{
