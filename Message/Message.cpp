...
static std::string uniqueString(void);

//----< default constructor results in Message with no attributes, but creates a unique id >----

Message::Message() 
{
	attributes_["id"] = uniqueString();
}
...
//----< get reply_to attribute >-------------------------------------------

std::string Message::reply_to()
{
	if (containsKey("reply_to"))
	{
		return attributes_["reply_to"];
	}
	return "";
}
//----< set reply_to attribute >-------------------------------------------

void Message::reply_to(const std::string& reply_to)
{
	attributes_["reply_to"] = reply_to;
}
...
Message::Value Message::get(const Message::Key& key) const
{
	if (containsKey(key))
	{
		auto val = attributes_.at(key);
		return val;
	}
	return "";
}
